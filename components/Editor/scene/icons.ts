...
import {
  ...
  Observable, PointerEventTypes, Vector2, AbstractMesh, ParticleSystem, IMouseEvent, MeshBuilder,
} from "@babylonjs/core";
...
export class SceneIcons {
  ...
  public constructor(editor: Editor) {
    ...
    // Materials
    this._lightMaterial = this._getIconMaterial("lightMaterial", "/icons/lightbulb.png");
    this._cameraMaterial = this._getIconMaterial("cameraMaterial", "/icons/camera.png");
    this._particleMaterial = this._getIconMaterial("particleMaterial", "/icons/wind.png");

    this._soundOnMaterial = this._getIconMaterial("soundOnMaterial", "/icons/volume-up.png");
    this._soundOffMaterial = this._getIconMaterial("soundOffMaterial", "/icons/volume-mute.png");
  ...
  private _createPlane(name: string): Mesh {
    const plane = MeshBuilder.CreatePlane(name, { size: 0.5 }, this._layer.utilityLayerScene);
